...
module.exports.pokedex=function(res){
	var pokemon=new Array();
	request('http://es.pokemon.wikia.com/wiki/Lista_de_Pok%C3%A9mon_por_color',function(err,resp,body){
		var poke=pokemon;		
		if(!err){
			$=cheerio.load(body);	
			var $pokemon=$("td > a:first-child");
			$pokemon.each(function(index, el) {
				var $img=$(this).children('img').attr('data-src');
				if($img==null){
					$img=$(this).children('img').attr('src')
				}
				var token={
					url:'http://es.pokemon.wikia.com'+ $(this).attr('href'),
					img:$img,
